...
export function FeedbacksPage() {
	...
	return (
		<section aria-labelledby='feedbacks-title'>
			<header className='mb-4 flex flex-col gap-3 md:flex-row md:items-end md:justify-between'>
				<div>
					<h1 id='feedbacks-title' className='text-xl font-semibold'>
						{tNav('feedbacks')}
					</h1>
				...
				<div className='flex w-full flex-col gap-2 md:max-w-3xl md:flex-row'>
					...
					<div className='md:w-56'>
						<label htmlFor='feedbacks-filter' className='block text-xs text-white/70'>
							{t('filter.label')}
						</label>
						<select
							id='feedbacks-filter'
					...
					<div className='flex gap-2 md:w-[28rem]'>
						<div className='flex-1'>
							<label htmlFor='feedbacks-date-from' className='block text-xs text-white/70'>
								{t('dateFilter.from')}
							</label>
							<input
						...
						<div className='flex-1'>
							<label htmlFor='feedbacks-date-to' className='block text-xs text-white/70'>
								{t('dateFilter.to')}
							</label>
							<input
			...
			{error && (
				<div role='alert' aria-live='assertive' className='mb-4 rounded-md border border-red-500/30 bg-red-500/10 p-3 text-sm text-red-200'>
			...
			<form onSubmit={handleCreate} aria-label={t('form')} className='mb-4 grid grid-cols-1 gap-3 md:grid-cols-7'>
				<div className='md:col-span-2'>
					<label htmlFor='c-author' className='block text-xs text-white/70'>
						{t('fields.author')}
					</label>
					<input id='c-author' value={createAuthor} onChange={e => setCreateAuthor(e.target.value)} required className='w-full rounded-md border border-white/10 bg-white/5 p-2 outline-none focus:ring-2 focus:ring-blue-500' />
				</div>
				<div className='md:col-span-3'>
					<label htmlFor='c-comment' className='block text-xs text-white/70'>
						{t('fields.comment')}
					</label>
					<input id='c-comment' value={createComment} onChange={e => setCreateComment(e.target.value)} required className='w-full rounded-md border border-white/10 bg-white/5 p-2 outline-none focus:ring-2 focus:ring-blue-500' />
				</div>
				<div>
					<label htmlFor='c-rating' className='block text-xs text-white/70'>
						{t('fields.rating')}
					</label>
					<input id='c-rating' type='number' min={1} max={5} value={createRating} onChange={e => setCreateRating(e.target.value === '' ? '' : Number(e.target.value))} required className='w-full rounded-md border border-white/10 bg-white/5 p-2 outline-none focus:ring-2 focus:ring-blue-500' />
				</div>
				<div>
					<label htmlFor='c-status' className='block text-xs text-white/70'>
						{t('fields.status')}
					</label>
					<select id='c-status' value={createStatus} onChange={e => setCreateStatus(e.target.value as FeedbackEntity['status'])} className='w-full rounded-md border border-white/10 bg-white/5 p-2 outline-none focus:ring-2 focus:ring-blue-500'>
						{statusOptions.map(o => (<option key={o.value} value={o.value}>{o.label}</option>))}
					</select>
				</div>
				<div className='md:self-end'>
					<button type='submit' disabled={creating || !createAuthor.trim() || !createComment.trim() || createRating === ''} className='rounded-md bg-blue-600 px-3 py-2 text-sm hover:bg-blue-500 disabled:opacity-50'>
						{t('addFeedback')}
					</button>
				</div>
			</form>
				...
				<table className='min-w-full text-sm' aria-label={tNav('feedbacks')}>
					<thead className='text-left text-white/70'>
						<tr>
							...
							<th scope='col' className='pb-2 pr-4'>{t('fields.author')}</th>
							<th scope='col' className='pb-2 pr-4'>{t('fields.comment')}</th>
							<th scope='col' className='pb-2 pr-4'>{t('fields.rating')}</th>
							...
							<tr key={f.id}>
								...
								<td className='py-2 pr-4'>
									{editingId === f.id ? (
										<input
											...
											className='w-20 rounded-md border border-white/10 bg-white/5 p-2 outline-none focus:ring-2 focus:ring-blue-500'
										/>
									) : (
								...
								<td className='py-2 pr-4'>
									{editingId === f.id ? (
										...
										<span>{mapStatus(f.status)}</span>
									)}
								</td>
								<td className='py-2 pr-4'>
									<span>{formatDateTime(f.time)}</span>
								</td>
								<td className='py-2'>
			...
			<div className='mt-4 flex flex-col items-center gap-3 md:flex-row md:justify-between'>
				<div className='flex items-center gap-2'>
					<button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={loading || (data?.totalPages ?? 1) <= 1 || page <= 1} className='rounded-md bg-white/5 px-3 py-1 text-sm hover:bg-white/10 disabled:opacity-50'>
						{t('previous')}
					</button>
					<p className='text-sm text-white/80'>
						{t('page')} {page} {t('of')} {data?.totalPages ?? 1}
					</p>
					<button onClick={() => setPage(p => p + 1)} disabled={loading || (data?.totalPages ?? 1) <= 1 || page >= (data?.totalPages ?? 1)} className='rounded-md bg-white/5 px-3 py-1 text-sm hover:bg-white/10 disabled:opacity-50'>
						{t('next')}
					</button>
				</div>
				<div className='flex items-center gap-2'>
					...
					<select id='page-size' value={pageSize} onChange={e => { setPage(1); setPageSize(Number(e.target.value)) }} className='rounded-md border border-white/10 bg-white/5 p-1.5 text-sm outline-none focus:ring-2 focus:ring-blue-500'>
